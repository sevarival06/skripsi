    ...
    <div class="row mt-5">
        <div class="col mb-5 mb-xl-0">
            <div class="card shadow">
                ...
                <div class="card-body">
                    <form action="{{ route('liabilitas.store') }}" method="POST">
                        @csrf
                        @if(!empty($id))
                        ...
                        <!-- Tanggal -->
                        <div class="form-group mb-3">
                            <label for="tgl_liabilitas" class="form-control-label">Tanggal</label>
                            <div class="input-group">
                                <input type="date" class="form-control @error('tgl_liabilitas') is-invalid @enderror" 
                                       id="tgl_liabilitas" name="tgl_liabilitas" 
                                       value="{{ old('tgl_liabilitas', $data->tgl_liabilitas ?? '') }}">
                                @error('tgl_liabilitas')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="text-center">
                            ...
                            <a href="{{ route('liabilitas.index') }}" class="btn btn-secondary">Kembali</a>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        if($('#tgl_liabilitas').length) {
            $('#tgl_liabilitas').datepicker({
                format: 'yyyy-mm-dd',
                autoclose: true,
                todayHighlight: true,
